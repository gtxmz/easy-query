...
public interface IEntityGroup1<T1Proxy extends ProxyEntity<T1Proxy, T1>, T1>  extends ClientEntityQueryableAvailable<T1> ,EntityQueryableAvailable<T1Proxy,T1> {
     ...
     * <blockquote><pre>
     * {@code
     * .groupBy(o -> GroupKeys.of(o.content().subString(0, 8))) //GROUP BY subString(content,1,8)具体的索引按各种数据库规则
     *
     * .groupBy(o -> GroupKeys.of(o.id())) // GROUP BY id
     *
     * .groupBy(t ->GroupKeys.of( t.id(),t.title())) // GROUP BY id,title
     *    }
     * </pre></blockquote>
     *
